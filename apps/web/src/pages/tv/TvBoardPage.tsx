...
import { useKiosk, usePrefersReducedMotion } from "./useKiosk";
...
export function TvBoardPage() {
  ...
  useEffect(() => {
    if (!celebration) {
      if (pendingAd.current && featured.length > 0) {
  ...
  return (
    <div className="relative flex h-[100dvh] flex-col overflow-hidden bg-mist text-ink">
      {!reducedMotion && <PeriodWipe period={period} gameType={gameType} visible={wipe} />}
      <Celebration round={celebration} reducedMotion={reducedMotion} />
      <FullscreenAd promo={fullscreenAd ? (activeAd ?? null) : null} reducedMotion={reducedMotion} />

      <header className="relative z-10 grid grid-cols-[1fr_auto_1fr] items-center gap-4 px-6 py-4">
        <div className="flex items-center">
          <img src="/logo-wordmark.svg" alt="Kšírovka" className="h-10 w-auto xl:h-12" />
        </div>
          ...
          <div className="flex items-center gap-2">
            {TV_GAMES.map((item) => (
              <button
                ...
                className={cn(
                  "rounded-full px-4 py-1 text-base font-semibold text-white",
        ...
        <p className="justify-self-end text-4xl font-semibold text-teal tabular-nums">{formatClock(now)}</p>
      </header>

      <main className="relative z-10 grid min-h-0 flex-1 grid-rows-[minmax(0,1.15fr)_minmax(0,0.95fr)] gap-5 px-6 pb-3">
        <section className="min-h-0">
          {emptyBoard ? (
            <EmptyMorning gameType={gameType} />
          ) : (
            <Podium leaderboard={stats?.playerLeaderboard ?? []} reducedMotion={reducedMotion} />
          )}
        </section>

        <section className="grid min-h-0 grid-cols-3 gap-5">
          <div className="tv-card flex min-h-0 flex-col p-5">
            <p className="text-sm font-semibold text-teal">
              {GAME_LABELS[gameType]} · {PERIOD_LABELS[period]} · 4–10
            </p>
            <ol className="mt-3 min-h-0 space-y-2 overflow-hidden">
              ...
              {rest.map((row) => (
                <li key={`${row.playerName}-${row.gameType}`} className="flex items-center gap-3">
                  <span className="w-8 text-2xl font-semibold text-teal">{row.rank}</span>
                  <span className="min-w-0 flex-1 truncate text-lg">{row.playerName}</span>
                  <span className="text-2xl font-semibold text-ink">{row.bestTotal}</span>
                </li>
              ))}
            </ol>
          </div>
              ...
              <ul className="mt-3 space-y-2">
                ...
                {recent.map((round) => (
                  <li key={round.id} className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-2xl font-semibold text-orange">{round.winnerTotal}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="tv-card min-h-0 flex-1 p-5">
